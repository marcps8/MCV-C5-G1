...
def get_data_sets_path(data_path):
    train_dir = os.path.join(data_path,"train_combined")
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if (test_acc > best_accuracy):
            ...
            torch.save({
                        ...
                        'loss': loss_fn,
                        }, 'model/best-model-parameters-augmented.pt')
...
def save_loss_curves(model_results):
    ...
    plt.xlabel('Epochs')
    plt.legend()
    plt.savefig("model/training_history_augmented.jpg")
...
def my_test_step(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, test_data):
    ...
    np.savetxt("model/predictions_test_set_augmented.csv",
        predictions,
        delimiter =",",
        fmt ='% s')
