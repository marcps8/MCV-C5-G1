        ...
        with open(instance_dir, "r") as f:
            for line in f:
                ...
                if frame not in frames or len(frames)==0 and True:
                    if len(frames) != 0 or len(dataset_dicts) != 0:
                    ...
                    for img in os.listdir(imgs_dir):
                        ...
                        record = {}
                        record["file_name"] = str(os.path.join(imgs_dir, img))
                        record["image_id"] = int(f"{v}{image_frame:05}")
                        id += 1
                        record["height"] = height
                        record["width"] = width
                        record["sem_seg_file_name"] = str(os.path.join(img_dir, "instances", v, img))
...
for entry in ann:
    if 'annotations' in entry:
        print("Annotations exist for image_id:", entry['image_id'])
    else:
        print("No annotations for image_id:", entry['image_id'])
"""
