...
from triplets_dataset import TripletsDatasetImg2Txt, TripletsDatasetVal, TripletsDataset
...
if __name__ == "__main__":
    ...
    db_text_embeddings_path = OUTPUT_PATH + f"/pickles/text_embeddings_train_25perc_txt2img_{args.text_model}.pkl"  # Changed to txt2img
    
    text_embeddings_path = OUTPUT_PATH + f"/pickles/text_embeddings_val_{args.text_model}.pkl"  # Changed to txt2img

    # to do afegir embeddings val
    query_features_path = OUTPUT_PATH + "/pickles" + f"/query_features_{args.mode}_{args.text_model}.pkl"  
    db_features_path = OUTPUT_PATH + "/pickles" + f"/db_features_{args.mode}_{args.text_model}.pkl"  
    ...
    if os.path.exists(triplets_path):
        ...
        triplets_val = get_triplets_from_text_to_image(None, load_triplets=True, output_path=triplets_path)
    ...
    if os.path.exists(triplets_train_path):
        ...
        triplets_train = get_triplets_from_text_to_image(None, load_triplets=True, output_path=triplets_train_path)

    if os.path.exists(text_embeddings_path) and os.path.getsize(text_embeddings_path) > 0 and os.path.exists(db_text_embeddings_path):
        ...
        with open(db_text_embeddings_path, "wb") as f:
            ...
            db_text_embeddings = generate_embeddings(triplets_train, config["text_model"])
            print("Generated!")
    ...
    train_dataset = TripletsDataset(triplets=triplets_train, root_dir=TRAIN_PATH, transform=get_transforms())
    val_loader = DataLoader(val_dataset, batch_size=config["batch_size"], shuffle=False, num_workers=8)
    train_loader = DataLoader(train_dataset, batch_size=config["batch_size"], shuffle=False, num_workers=8)
    net = Network(config, val_loader=val_loader, train_loader=train_loader, text_embeddings=text_embeddings)
    net.load_model()
    ...
    for feature in db_features:
        ...
        feature_tensor = torch.tensor(feature, device=net.device)  # Convert numpy array to PyTorch tensor
    ...
    f_query_features = [tensor.flatten().cpu().detach().numpy() for tensor in f_query_features]
    ...
    for i in range(len(indices)):
        ...
        print(f"Train Caption(first): {results['train_captions'][results['train_id'][i][0]]}")
